#!/usr/bin/env python3
"""Match Nahar Fasiləsi gallery to local forum photos; copy to LunchBreak/."""
...
def load_gallery_urls() -> list[str]:
    if not PAGE_CACHE.is_file():
        raise FileNotFoundError("Run _fetch_lunch_break_gallery_playwright.py first")
...
def hash_thumb(index: int) -> int:
    return hash_file(THUMB_DIR / f"{index:02d}.jpg")
