#!/usr/bin/env python3
"""Remove page banner; align Garabagh outputs with gallery order."""
...
OUT_DIR = ROOT / "images" / "photos-gallery" / "Garabagh"
...
MD = ROOT / "helpers" / "_cache_garabagh_page.md"
BANNER_BYTES = 682043
...
def main() -> None:
    ...
    for new_i in range(1, len(urls) + 1):
        old_i = new_i + 1
        old_dest = OUT_DIR / f"garabagh-{old_i:02d}.jpg"
        new_dest = tmp / f"garabagh-{new_i:02d}.jpg"
        if old_dest.is_file():
    ...
    for old in OUT_DIR.glob("garabagh-*.jpg"):
        old.unlink()
