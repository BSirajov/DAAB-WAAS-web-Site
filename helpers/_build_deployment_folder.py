...
def write_deployment_readme(
    counts: dict[str, int], dest_dir: Path, *, skip_images: bool
) -> None:
    images_row = (
        "| `images/` | **Not copied** — unchanged on server; keep existing production folder |\n"
        if skip_images
        else "| `images/` | Photos, logos, QR codes, gallery |\n"
    )
...
{images_row}| `i18n/` | Navigation, routes, search, UI strings |
| `forum_2024/*.pdf` | Forum book download |
| `sitemap.xml` | SEO sitemap (if present) |
