...
def write_deployment_readme(counts: dict[str, int], dest_dir: Path) -> None:
...
"""
    (dest_dir / "README-DEPLOY.txt").write_text(text, encoding="utf-8")
...
def replace_deploy_dir(staging: Path, target: Path) -> None:
...
def main() -> int:
    ...
    write_deployment_readme({"files": copied}, DEPLOY_STAGING)
    ...
    replace_deploy_dir(DEPLOY_STAGING, DEPLOY_DIR)
