#!/usr/bin/env python3
"""Remove page banner; align Portraits outputs with gallery order."""
...
MD = ROOT / "helpers" / "_cache_portraits_page.md"
BANNER_BYTES = 682043
...
def main() -> None:
    ...
    for new_i in range(1, len(urls) + 1):
        old_i = new_i + 1
        old_dest = OUT_DIR / f"portraits-{old_i:03d}.jpg"
        new_dest = tmp / f"portraits-{new_i:03d}.jpg"
        if old_dest.is_file():
        ...
        new_manifest.append(
            {
                ...
                "dest": f"portraits-{new_i:03d}.jpg",
    ...
    for old in OUT_DIR.glob("portraits-*.jpg"):
        old.unlink()
