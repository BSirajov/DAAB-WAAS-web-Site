#!/usr/bin/env python3
"""Remove page banner; align LunchBreak outputs with markdown gallery order."""
...
OUT_DIR = ROOT / "images" / "photos-gallery" / "LunchBreak"
MANIFEST = OUT_DIR / "_match-manifest.json"
URL_CACHE = ROOT / "helpers" / "_cache_lunch_break_gallery_urls.txt"
THUMB_DIR = ROOT / "helpers" / "_cache_lunch_break_gallery_thumbs"
MD = ROOT / "helpers" / "_cache_lunch_break_page.md"
BANNER_BYTES = 682043
...
def main() -> None:
    ...
    for new_i in range(1, len(urls) + 1):
        old_i = new_i + 1
        old_dest = OUT_DIR / f"lunch-break-{old_i:02d}.jpg"
        new_dest = tmp / f"lunch-break-{new_i:02d}.jpg"
        if old_dest.is_file():
        ...
        new_manifest.append(
            {
                ...
                "dest": f"lunch-break-{new_i:02d}.jpg",
    ...
    for old in OUT_DIR.glob("lunch-break-*.jpg"):
        old.unlink()
