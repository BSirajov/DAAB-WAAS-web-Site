#!/usr/bin/env python3
"""
Match Google Sites Açılış günü gallery images to local forum photos by perceptual hash,
then copy originals into images/photos-gallery/OpeningDay/.
"""
...
OUT_DIR = ROOT / "images" / "photos-gallery" / "OpeningDay"
MANIFEST = OUT_DIR / "_match-manifest.json"
PAGE_CACHE = ROOT / "helpers" / "_cache_opening_day_gallery_urls.txt"
THUMB_DIR = ROOT / "helpers" / "_cache_opening_day_gallery_thumbs"
LOCAL_HASH_CACHE = ROOT / "helpers" / "_cache_alley_local_hashes.json"
DEST_PREFIX = "opening-day"
MAX_DIST = 14
RETRY_MAX_DIST = 19
...
def load_gallery_urls() -> list[str]:
    if not PAGE_CACHE.is_file():
        raise FileNotFoundError(f"Missing {PAGE_CACHE} — run _fetch_opening_day_gallery_playwright.py")
...
def main() -> None:
    ...
    if not THUMB_DIR.is_dir() or not any(THUMB_DIR.glob("*.jpg")):
        raise SystemExit("Run: python helpers/_fetch_opening_day_gallery_playwright.py")
