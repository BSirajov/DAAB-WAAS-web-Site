#!/usr/bin/env python3
"""
Match Google Sites Müzakirələr gallery images to local forum photos by perceptual hash,
then copy originals into images/photos-gallery/Discussions/.
"""
...
def load_gallery_urls() -> list[str]:
    if not PAGE_CACHE.is_file():
        raise FileNotFoundError(
            f"Missing {PAGE_CACHE} — run _sync_discussions_urls_from_page.py"
        )
    return [ln.strip() for ln in PAGE_CACHE.read_text(encoding="utf-8").splitlines() if ln.strip()]


def thumb_path(index: int) -> Path:
    return THUMB_DIR / f"{index:03d}.jpg"


def dest_name(index: int, suffix: str) -> str:
    return f"{DEST_PREFIX}-{index:03d}{suffix.lower()}"
...
def main() -> None:
    ...
    if not THUMB_DIR.is_dir() or not any(THUMB_DIR.glob("*.jpg")):
        raise SystemExit("Run: python helpers/_fetch_discussions_gallery_playwright.py")
    ...
    for i, url in enumerate(urls, start=1):
        ...
        if match_path is None or dist > MAX_DIST:
            if not (dist <= RETRY_MAX_DIST and match_path is not None):
                print(f"  NO MATCH #{i} (best dist={dist})")
                manifest.append(
        ...
        used.add(match_path)
        name = dest_name(i, match_path.suffix)
        dest = OUT_DIR / name
    ...
    unmatched = [m for m in manifest if m.get("matched") is False]
    print(f"\nDone: {matched}/{len(urls)} matched -> {OUT_DIR}")
    if unmatched:
        print(f"Unmatched: {len(unmatched)} — see {MANIFEST}")
    else:
